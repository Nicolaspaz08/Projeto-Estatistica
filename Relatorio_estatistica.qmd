...
library(corrplot)
library(ggplot2)
library(report) # Para relatórios descritivos de modelos
library(gtsummary) # Para tabelas de regressão bonitas
library(easystats) # Para checagem de pressupostos
library(kableExtra)

# Pode ser necessário instalar alguns pacotes se ainda não tiver:
...
Essa limpeza em duas etapas é um passo crucial para assegurar que o modelo de regressão seja construído sobre uma base de dados robusta e confiável.
...
Após a limpeza e correção dos dados, a análise de outliers foi realizada no conjunto de dados final (`dados`). O objetivo desta etapa é identificar visualmente (com boxplots) e numericamente os valores que se distanciam significativamente da maioria das observações para cada variável. A influência desses pontos será verificada posteriormente, na fase de diagnóstico do modelo de regressão.

```{r}
# Define a função para mostrar o boxplot e listar os outliers
mostrar_outliers <- function(dataframe, nome_da_coluna) {
  
  # Seleciona o vetor de dados da coluna
...
-   **Tecnologia:** A nossa amostra de usinas está bem distribuída entre as quatro categorias tecnológicas. Não há um tipo de usina que domine desproporcionalmente a base de dados, o que é positivo, pois permite que o modelo de regressão avalie o efeito de cada tecnologia.
...
A análise da matriz de correlação revela insights cruciais e surpreendentes sobre os dados:
...
# Executando a checagem de colinearidade
...
**Por que `tecnologia` tem um VIF tão alto?** Isso acontece porque os valores de outras variáveis estão fortemente associados a certas categorias de `tecnologia`. Por exemplo, usinas do tipo "Solar" sempre terão alta `irradiancia_kWh_m2_dia` e baixo `vento_m_s`, enquanto usinas "Eólicas" terão o oposto. Isso faz com que a variável `tecnologia` carregue informações que já estão presentes em outras variáveis.
...
## **4.3. Apresentação e Interpretação do Modelo Final**
...
-   **Ajuste Geral do Modelo:** O gráfico de "Posterior Predictive Check" mostra que a distribuição dos dados previstos pelo modelo (linha verde) se alinha quase perfeitamente com a distribuição dos dados reais (linha azul), indicando um excelente ajuste geral do modelo à realidade dos dados.
    ...
    ## **5.2.** Análise Detalhada com a Função `plot()`
...
O modelo inicial, que incluía todas as variáveis preditoras, revelou um forte problema de multicolinearidade entre `capacidade_instalada_MW` e `investimento_R$mi`, confirmado por uma correlação de 0.95 e altos valores de VIF. Para resolver essa questão, foi adotada uma abordagem metodológica de comparação entre dois modelos reduzidos. O modelo que removeu a variável `investimento_R$mi` se mostrou estatisticamente superior em todas as métricas avaliadas (maior R² Ajustado, menores AIC e BIC), resultando em um modelo final mais robusto, parcimonioso e com coeficientes interpretáveis.
...
Embora o modelo apresente forte aderência aos dados, recomenda-se, para estudos futuros, a exploração de possíveis interações entre variáveis e a avaliação de modelos não lineares. Ainda assim, esta análise fornece análises valiosas para a otimização de sistemas de energia renovável, contribuindo para decisões mais embasadas no setor.
